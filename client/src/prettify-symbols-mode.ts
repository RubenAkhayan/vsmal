...
export function prettyTextToString(txt: text.AnnotatedText) : string {
  ...
  // `matop` loves outputting non-breaking spaces instead of normal whitespace, so we replace them
  ...
  if(psm) {
    psm.activate()
      .then(() => {
        ...
        subscriptions.push(psm.exports.registerSubstitutions({language: 'ma', substitutions: [{ugly: 'SOLIDITY', pretty: '⫝'}]}));
      });
  }
